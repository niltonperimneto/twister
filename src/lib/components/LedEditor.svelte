...
<script lang="ts">
    ...
    function needsTertiary(mode: number): boolean {
        return mode === 6;
    }
    function needsDuration(mode: number): boolean {
        return mode >= 2;
    }
    ...
    const colorWrite = makeDebounce(async (led: LedDto, hex: string) => {
        ...
        try {
            ...
            if (idx >= 0)
                localLeds[idx].color = profile.leds[idx]?.color ?? rgb;
        }
    }, 120);
    ...
    const brightnessWrite = makeDebounce(async (led: LedDto, value: number) => {
        ...
        try {
            ...
            if (idx >= 0)
                localLeds[idx].brightness =
                    profile.leds[idx]?.brightness ?? value;
        }
    }, 120);
    ...
    const durationWrite = makeDebounce(async (led: LedDto, ms: number) => {
        ...
        try {
            ...
            if (idx >= 0)
                localLeds[idx].effect_duration =
                    profile.leds[idx]?.effect_duration ?? ms;
        }
    }, 120);
    ...
    async function handleModeChange(led: LedDto, mode: number) {
        ...
        try {
            ...
            addToast("LED mode write failed");
        }
    }
</script>
            ...
            >
                <div class="flex items-center gap-2.5">
                    ...
                    <span class="text-[10px] text-base-content/35 font-mono"
                        >{LED_MODES[led.mode] ?? "Unknown"}</span
                    >
                </div>
                <svg
                    class="w-3.5 h-3.5 opacity-40 transition-transform duration-200 {isOpen
                        ? 'rotate-180'
                        : ''}"
                    viewBox="0 0 24 24"
                    fill="none"
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <div class="pill-group">
                            {#each led.modes as modeId (modeId)}
                                <button
                                    class="pill-btn {led.mode === modeId
                                        ? 'pill-btn-active'
                                        : ''}"
                                    onclick={() =>
                                        handleModeChange(sourceLed, modeId)}
                                >
                    ...
                    {#if led.mode !== 0}
                                ...
                                <div class="flex flex-col gap-3 flex-1 min-w-0">
                                    ...
                                    <div class="flex flex-col gap-1">
                                        ...
                                        <div class="led-hex-field">
                                            <span
                                                class="text-base-content/25 text-xs select-none"
                                                >#</span
                                            >
                                            <input
                                                type="text"
                                    ...
                                    {#if needsSecondary(led.mode)}
                                        <div class="flex items-center gap-2">
                                            <span
                                                class="text-[10px] text-base-content/30 w-6"
                                                >2nd</span
                                            >
                                            <div
                                                class="w-4 h-4 rounded border border-white/6"
                                                style="background: {rgbToHex(
                                                    led.secondary_color,
                                                )};"
                                            ></div>
                                            <span
                                                class="text-[10px] font-mono text-base-content/25"
                                                >{rgbToHex(
                                                    led.secondary_color,
                                                ).toUpperCase()}</span
                                            >
                        ...
                        <div class="flex flex-col gap-1.5">
                            <div class="flex items-center justify-between">
                                ...
                                >
                                <span
                                    class="text-xs font-mono text-base-content/35 tabular-nums"
                                    >{Math.round(
                                        (led.brightness / 255) * 100,
                                    )}%</span
                                >
                            </div>
                            <input
